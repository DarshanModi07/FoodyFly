...
const Items = ({ items, resId, title, onCartUpdate }) => {
  ...
  const handleAddItem = async (item) => {
    ...
    if (!isVerified) { toast.error("Please login to add items 🔒"); return; }

    const loadingToast = toast.loading("Adding item...");
    setUpdatingItem(item.name);
    try {
      await axios.post(BASE_URL + "orderAdd", {
        id: resId ?? item.resId,
        item: item.name,
  ...
  const handleRemoveItem = async (item) => {
    const isVerified = await verifyUser();
    if (!isVerified) { toast.error("Please login to modify cart 🔒"); return; }

    const loadingToast = toast.loading("Removing item...");
    setUpdatingItem(item.name);
    try {
      await axios.post(BASE_URL + "orderDelete", {
        id: resId ?? item.resId,
        item: item.name,
        categories: title ?? item.categories,
      }, { withCredentials: true });
  ...
  return (
    <div className="p-2">
      {items.map((item, index) => (
        <div key={index} className="flex flex-col md:flex-row justify-between bg-white m-2 p-4 rounded-lg shadow-sm dark:bg-gray-600 dark:text-white">
